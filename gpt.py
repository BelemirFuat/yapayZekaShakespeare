...
# Hiperparametreler
batch_size = 128  # Paralel işlem yapacağımız bağımsız dizi sayısı
block_size = 128  # Tahminler için maksimum bağlam uzunluğu
max_iters = 5000  # Toplam eğitim adımı sayısı
eval_interval = 500  # Değerlendirme için kaç adımda bir kontrol yapılacak
learning_rate = 3e-4  # Öğrenme oranı
device = 'cuda' if torch.cuda.is_available() else 'cpu'  # Cihaz, eğer GPU varsa CUDA kullan
eval_iters = 200  # Değerlendirme için kaç örnek alınacak
n_embd = 384  # Gömülü (embedding) boyutu
n_head = 6  # Çoklu başlık sayısı (multi-head attention)
n_layer = 6  # Transformer katman sayısı
...
for iter in range(max_iters):
    ...
    # Modeli her eval_interval'da değerlendiriyoruz
    losses = estimate_loss()
    print(f"Adım {iter}: eğitim kaybı {losses['train']:.4f}, doğrulama kaybı {losses['val']:.4f}")
    ...
    end_time = time.time()
    print(f"Bir iterasyonun süresi: {end_time - start_time:.2f} saniye")
    file.write(f"{end_time - start_time:.2f}\n")  # Süreyi dosyaya kaydediyoruz
file.close()

# Modelden metin üretme
context = torch.zeros((1, 1), dtype=torch.long, device=device)
print(decode(m.generate(context, max_new_tokens=500)[0].tolist()))  # Üretilen metni yazdırıyoruz
